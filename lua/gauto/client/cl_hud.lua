function GAuto.Notify( text )
	local color1 = Color( 180, 0, 0, 255 )
	local color2 = color_white
	chat.AddText( color1, "[GAuto]: ", color2, text )
end
...
hook.Add( "HUDPaint", "GAuto_HUDStuff", function()
	local disabled = cvars.Bool( "gauto_disable_hud" )
	if disabled then return end

	local ply = LocalPlayer()
	local vehicle = ply:GetVehicle()
	if !ply:InVehicle() or !GAuto.IsDrivable( vehicle ) then return end

	local hp = vehicle:GetNWInt( "GAuto_VehicleHealth" )
	local maxHp = vehicle:GetNWInt( "GAuto_VehicleMaxHealth" )
	local god = vehicle:GetNWBool( "GodMode" )
	...
	if maxHp > 0 then
		local percent = CalcPercentage( hp, maxHp )
