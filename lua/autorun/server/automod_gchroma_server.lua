	...
	if gchroma then
		if !veh:GetNWBool( "IsAutomodSeat" ) and veh.seat then
			...
			for k,v in pairs( veh.seat ) do
				...
				gchroma.SetDeviceColorEx( GCHROMA_DEVICE_KEYBOARD, color, _G["GCHROMA_KEY_"..k + 1], 0 ) --Seat keys will light up red if they are occupied
			end
	...
	if gchroma then
		if veh:GetNWBool( "IsAutomodSeat" ) then
			...
			if IsValid( driver ) then
				gchroma.SetDeviceColorEx( GCHROMA_DEVICE_KEYBOARD, GCHROMA_COLOR_RED, _G["GCHROMA_KEY_"..veh.ID + 1], 0 )
				gchroma.SendFunctions( driver )
			end
		else
...
local function AM_GChromaLeaveVehicle( ply, ent )
	if gchroma then
		if ent:GetNWBool( "IsAutomodSeat" ) then
			...
			local driver = parent:GetDriver()
			if IsValid( driver ) then
				gchroma.SetDeviceColorEx( GCHROMA_DEVICE_KEYBOARD, GCHROMA_COLOR_WHITE, _G["GCHROMA_KEY_"..ent.ID + 1], 0 )
				gchroma.SendFunctions( driver )
			end
		else
