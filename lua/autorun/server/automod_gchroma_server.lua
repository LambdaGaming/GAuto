	...
	if GChroma_Loaded then
		if !veh:GetNWBool( "IsAutomodSeat" ) and veh.seat then
			...
			for k,v in pairs( veh.seat ) do
				...
				GChroma_SetDeviceColorEx( ply, GCHROMA_DEVICE_KEYBOARD, Vector( 255, 0, 0 ), GCHROMA_KEY_1, 0 )
	...
	if GChroma_Loaded then
		if veh:GetNWBool( "IsAutomodSeat" ) then
			...
			if IsValid( driver ) and GChroma_Loaded then
				GChroma_SetDeviceColorEx( driver, GCHROMA_DEVICE_KEYBOARD, Vector( 255, 0, 0 ), _G["GCHROMA_KEY_"..veh.ID + 1], 0 )
			end
		else
...
local function AM_GChromaLeaveVehicle( ply, ent )
	if GChroma_Loaded then
		if ent:GetNWBool( "IsAutomodSeat" ) then
			...
			local driver = parent:GetDriver()
			if IsValid( driver ) and GChroma_Loaded then
				GChroma_SetDeviceColorEx( driver, GCHROMA_DEVICE_KEYBOARD, Vector( 255, 255, 255 ), _G["GCHROMA_KEY_"..ent.ID + 1], 0 )
			end
		else
